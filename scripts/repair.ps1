...
$aliases = [ordered]@{
  'EXPO_PUBLIC_FIREBASE_API_KEY' = @('EXPO_NEXT_PUBLIC_FIREBASE_API_KEY','NEXT_PUBLIC_FIREBASE_API_KEY')
  'EXPO_PUBLIC_FIREBASE_AUTH_DOMAIN' = @('EXPO_NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN','NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN')
  'EXPO_PUBLIC_FIREBASE_DATABASE_URL' = @('EXPO_NEXT_PUBLIC_FIREBASE_DATABASE_URL','EXPO_NEXT_PUBLIC_FIREBASE_DB_URL','NEXT_PUBLIC_FIREBASE_DATABASE_URL','NEXT_PUBLIC_FIREBASE_DB_URL','FIREBASE_DB_URL','CLOUD_FIREBASE_DB_URL')
  'EXPO_PUBLIC_FIREBASE_PROJECT_ID' = @('EXPO_NEXT_PUBLIC_FIREBASE_PROJECT_ID','NEXT_PUBLIC_FIREBASE_PROJECT_ID','FIREBASE_PROJECT_ID')
  'EXPO_PUBLIC_FIREBASE_STORAGE_BUCKET' = @('EXPO_NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET','NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET')
  'EXPO_PUBLIC_FIREBASE_MESSAGING_SENDER_ID' = @('EXPO_NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID','NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID')
  'EXPO_PUBLIC_FIREBASE_APP_ID' = @('EXPO_NEXT_PUBLIC_FIREBASE_APP_ID','NEXT_PUBLIC_FIREBASE_APP_ID')
}

$added = @()
foreach ($target in $aliases.Keys) {
  if ($envValues.ContainsKey($target) -and -not [string]::IsNullOrWhiteSpace($envValues[$target])) { continue }
...
foreach ($name in $requiredEnv) {
  if (-not $envValues.ContainsKey($name) -or [string]::IsNullOrWhiteSpace($envValues[$name])) { $missing += $name }
}
